...
export class NotarizationXVerifiedFollowers extends NotarizationBase {
  requestRecorder: RequestRecorder = new RequestRecorder(
    [{ method: "GET", urlPattern: "https://x.com/i/api/graphql/LwtiA7urqM6eDeBheAFi5w/AccountOverviewQuery?*" }],
  ...
  private async onRequestsCaptured(log: Array<Request>) {
    ...
    const newVariablesParam = encodeURIComponent(JSON.stringify(minimalVariables))
    ...
    const verifiedFollowersMatch = responseBody.match(/"verified_follower_count":"(\d+)"/);
