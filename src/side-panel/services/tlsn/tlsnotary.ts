...
import {
  ParsedHTTPMessage,
  parseHttpMessage,
} from '../../common/helpers/httpParser';
...
export class TLSNotary extends Progressive<Status> {
  ...
  async transcript(
    ...
    if (this.state.status === Status.InProgress)
      return new Error('Notarization is in progress');
    ...
    if (resp.status !== 200)
      return new Error(`Notarization failed with status ${resp.status}`);

    const transcript = await this.#prover.transcript();
    const parsedHttpMessage = parseHttpMessage(
