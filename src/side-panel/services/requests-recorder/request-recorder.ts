...
export class RequestRecorder extends Progressive<Status> {
  ...
  public get completedRequestsLog(): Array<Request> {
    return this.#completedRequestsLog;
  }
  ...
  start(): void {
    ...
    this.#completedRequestsLog = new Array<Request>(
      this.#targetRequests.length,
    );
    this.#requestsLog = new Array<PartialRequestLog | undefined>(
      this.#targetRequests.length,
    ).fill(undefined);
  ...
  private requestId(method: string, url: string): number {
    ...
    return this.#targetRequests.findIndex((tr) => {
  ...
  private completeRequestLog(
    ...
    if (toCompleteLog(existingLog)) {
      ...
      if (requestRes instanceof Error) {
        ...
        this.setError(
          new Error(
            `Failed to convert request log to Request object: ${requestRes.message}`,
          ),
        );
        return;
      }
  ...
  private updateRequestLog(
    requestId: number,
    updates: RequestBodyHandlerData | RequestHeadersHandlerData,
  ): Result<void> {
    if (requestId < 0 || requestId >= this.#targetRequests.length) return;
    const existingLog: PartialRequestLog | undefined =
      this.#requestsLog[requestId];
    ...
    if (existingLog && existingLog.requestId === updates.requestId) {
      ...
      Object.assign<
        PartialRequestLog,
        RequestBodyHandlerData | RequestHeadersHandlerData
      >(existingLog, updates);
      this.completeRequestLog(requestId, existingLog);
    } else if (
      !existingLog &&
      this.#completedRequestsLog[requestId] === undefined
    ) {
